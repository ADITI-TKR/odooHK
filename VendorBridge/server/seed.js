...
async function seed() {
  ...
  console.log('Creating 100 vendor user accounts (unique email + password each)...')
  ...
  const rfqDocs = Array.from({ length: 100 }, (_, i) => {
    ...
    return {
      ...
      items: [{
        ...
        specifications: `Standard enterprise ${itemName.toLowerCase()}`,
      }],
  ...
  for (let i = 0; i < 80; i++) {
    ...
    quotations.push({
      ...
      deliveryTimeline: `${rand(5, 30)} days`,
  ...
  console.log('\n✅ Seed complete!')
  console.log(`   ${companies.length} companies (unique company emails)`)
  console.log(`   ${vendors.length} vendors with login accounts`)
  console.log(`   ${productDocs.length} products with photos`)
  console.log(`   ${rfqs.length} RFQ requests`)
  console.log('\n📋 Staff logins:')
  console.log('   Admin:    admin@vendorbridge.com / admin')
  console.log('   Officer:  priya@vendorbridge.com / priya')
  console.log('   Manager:  manager@vendorbridge.com / manager')
  console.log('\n📋 Sample vendor logins (all 100 saved to vendor-credentials.json):')
  credentials.slice(0, 5).forEach((c) => {
    console.log(`   ${c.vendor}`)
    console.log(`     Company email: ${c.companyEmail}`)
    console.log(`     Login email:   ${c.loginEmail}`)
    console.log(`     Password:      ${c.password}`)
    console.log(`     Products:      ${c.products}`)
  })
  console.log('   ... and 95 more in vendor-credentials.json\n')
  process.exit(0)
}
