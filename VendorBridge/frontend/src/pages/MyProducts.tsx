...
export default function MyProducts() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const body = {
      ...form,
      images: [getProductImage(form.category || form.productName || '')],
    }
  ...
  return (
    <div>
      <PageHeader
        title="My Products"
        subtitle="Manage your product catalog — pricing, stock & MOQ."
        action={
          <button onClick={() => { setShowForm(true); setEditing(null); setForm(empty()) }} className="flex items-center gap-2 rounded-xl bg-brand px-5 py-2.5 text-sm font-semibold text-white">
        ...
        <form onSubmit={handleSave} className="mb-6 rounded-2xl border border-gray-100 bg-card p-6 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <img src={resolveProductImage(form.images, form.category || form.productName)} alt="Preview" className="h-32 w-full rounded-lg object-cover" />
            </div>
            ...
            <input type="number" placeholder="MOQ (Min Order Qty)" value={form.moq || ''} onChange={(e) => setForm({ ...form, moq: +e.target.value })} className="rounded-lg border px-4 py-2.5 text-sm outline-none focus:border-brand" />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          ...
          <div key={p._id} className="overflow-hidden rounded-2xl border border-gray-100 bg-card shadow-sm transition-shadow hover:shadow-md">
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <StatusBadge status={p.availability || 'Active'} />
              </div>
              ...
              <div className="mt-3 flex gap-2">
                <button onClick={() => startEdit(p)} className="flex flex-1 items-center justify-center gap-1 rounded-lg border py-2 text-xs font-medium hover:bg-gray-50">
                  <Edit size={14} /> Edit
                </button>
                <button onClick={() => handleDelete(p._id)} className="flex items-center justify-center rounded-lg border border-red-200 px-3 py-2 text-red-500 hover:bg-red-50">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
        ...
        {products.length === 0 && (
          <div className="col-span-full flex flex-col items-center py-16 text-muted">
            ...
            <p>No products yet. Add your first product to the catalog.</p>
          </div>
        )}
      </div>
    </div>
